...
2. Generate **at least 10 attack vectors**. Consider:
   ...
   - Injection (regex special chars in input, header injection)
   - Oversized payloads (above the documented limit, exactly at the limit, off-by-one)
   - Missing or wrong-typed fields in the request body
   - Empty / null / whitespace-only inputs
   - Race conditions if applicable (concurrent requests, shared state)
   - Off-by-one in any boundary (length, count, range)
   - Dependency assumptions that may not hold (regex engine, library versions)
...
5. Run `pytest -xvs tests/unit/test_adversarial.py` and confirm the tests fail (proving the bugs).
