...
- Pause when you see test-gaming (assertions edited to match broken output) or `try/except` swallowing.
- Keep `SPEC.md` updated in the same edit as any behavior change. That's the contract.
- The model can run forever. You need to know when to stop the loop and review.
...
- It's not a one-way ladder. The skill is moving up *and down* on purpose.
